...
class compress_jpeg(nn.Module):
    ...
    """
    def __init__(self, rounding=torch.round, factor=1, concat_dim=1):
    ...
    def forward(self, x):
        ...
        for k in components.keys():
            ...
            components[k] = comp
        y_hat = torch.cat((components['y'], components['cb'], components['cr']), dim=self.concat_dim)
        shapes = [
            components['y'].shape[self.concat_dim], 
            components['cb'].shape[self.concat_dim], 
            components['cr'].shape[self.concat_dim]
            ]
        return y_hat, shapes
